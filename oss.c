...
const int QUEUE_AMOUNT = 3; //amount of queues needed -1
...
//here
		...
		{
			if (isEmpty(active[0]) == 0) //there is something in queue 0, we are traversing these queues in the order of importance
			{
				activeProcIndex = FindLocPID(dequeue(active[0])); //dequeue the proccess
			...
			else if (isEmpty(active[1]) == 0) //there is something in queue 1 but not queue 0
			{
				activeProcIndex = FindLocPID(dequeue(active[1])); //dequeue the proccess
				msgbuf.mtype = data->proc[activeProcIndex].pid;
				strcpy(msgbuf.mtext, "");
			...
			else if (isEmpty(active[2]) == 0) //there is something in queue 2 but not in 1 or 0
			{
				activeProcIndex = FindLocPID(dequeue(active[2])); //dequeue the proccess
				msgbuf.mtype = data->proc[activeProcIndex].pid;
				strcpy(msgbuf.mtext, "");
			...
			else if (isEmpty(active[3]) == 0) //there is something in queue 3 but not in queue 1 or 0 or 2
			{
				activeProcIndex = FindLocPID(dequeue(active[3])); //dequeue the proccess
				msgbuf.mtype = data->proc[activeProcIndex].pid;
				strcpy(msgbuf.mtext, "");
